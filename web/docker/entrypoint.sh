...
if [ "$ROS_DISTRO" = 'noetic' ]; then
    ...
    ros2 launch rosbridge_server rosbridge_websocket_launch.xml "topics_glob:=$topics_glob" "services_glob:=$services_glob" "params_glob:=$params_glob" "actions_glob:=$actions_glob" "default_call_service_timeout:=5.0" "call_services_in_new_thread:=true" "send_action_goals_in_new_thread:=true" &
